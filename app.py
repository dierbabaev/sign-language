...
def main():
    ...
    # Camera preparation - Подготовка камеры ###############################################################
    ...
    while True:
        ...
        if temp != -1:
            ...
            temp, mode = select_mode(key, mode)
            number = temp
        ...
        img = np.zeros((1080, 750, 3), np.uint8)
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if symbol == '-':
                    ...
                    if not (len(result) > 0 and result[-1] == ' ' and symbol == ' '):
                        result += symbol
                    text = ''
        ...
        cv.putText(img, result, (50, 50), cv.FONT_HERSHEY_SIMPLEX, 0.7, (b, g, r), 1, cv.LINE_AA)
        cv.imshow("res", img)
    ...
    for index, landmark in enumerate(landmark_point):
        ...
        if index == 12:  # средний палец: кончик пальца
            cv.circle(image, (landmark[0], landmark[1]), 8, (255, 255, 255),
                      -1)
            cv.circle(image, (landmark[0], landmark[1]), 8, (0, 0, 0), 1)
        if index == 13:  # безымяный палец: корень
            cv.circle(image, (landmark[0], landmark[1]), 5, (255, 255, 255),
                      -1)
            cv.circle(image, (landmark[0], landmark[1]), 5, (0, 0, 0), 1)
        if index == 14:  # безымяный палец: 2-й сустав
            cv.circle(image, (landmark[0], landmark[1]), 5, (255, 255, 255),
                      -1)
            cv.circle(image, (landmark[0], landmark[1]), 5, (0, 0, 0), 1)
        if index == 15:  # безымяный палец: 1-й сустав
            cv.circle(image, (landmark[0], landmark[1]), 5, (255, 255, 255),
                      -1)
            cv.circle(image, (landmark[0], landmark[1]), 5, (0, 0, 0), 1)
        if index == 16:  # безымяный палец: кончик пальца
            cv.circle(image, (landmark[0], landmark[1]), 8, (255, 255, 255),
                      -1)
            cv.circle(image, (landmark[0], landmark[1]), 8, (0, 0, 0), 1)
